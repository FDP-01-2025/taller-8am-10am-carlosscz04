...
ifstream archivo("estudiantes.txt");
    ofstream eliminarEstudiante("temp.txt");
    Estudiante e;
    string BuscarCarnet;
    ...
    if (archivo.is_open() && eliminarEstudiante.is_open()) {
        while (archivo >> e.nombre >> e.carnet >> e.edad) {
            if (e.carnet == BuscarCarnet) {
          
               eliminarEstudiante << e.nombre << e.carnet <<  e.edad << endl;               
                eliminado = true;
            }
        }
        archivo.close();
